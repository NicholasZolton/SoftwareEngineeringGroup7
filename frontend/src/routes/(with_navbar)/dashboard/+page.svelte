<script lang="ts">
	...
	async function getUserEvents() {
		...
		let response: any = await fetch('http://127.0.0.1:5000/get_my_events', options)
...
<main>
	{#if $user == -1}
		...
		<h1><a href="/login">Log in now to create and view your events!</a></h1>	
	{:else if userEvents === null}
		<h1>Loading...</h1>
	{:else if userEvents.length == 0}
		<h1>This is where you can see all your events.</h1>
		<h1>However, it looks like you don't have any yet.</h1>
		<h1><a href="/create_event">Create one now!</a></h1>
	{:else}
		<div id="main-form">
			<div id="user-icon">
				<a href="/profile"><UserIcon css={"width: 30px; height: 30px;"}></UserIcon></a>
			</div>
			<h1>These are all your events:</h1>
			{#each userEvents as event}
				<div class="event">
					<a href="/view_event?id={event[0]}"><h2>{event[1]}</h2></a>
					<h2>{event[2].substring(0, event[2].length - 3)}</h2>
					<button on:click={() => {goto('/edit_event?id=' + event[0])}}>Manage</button>
				</div>
			{/each}
			<button id="create-event-button" on:click={() => {goto('/create_event')}}>Create a new Event!</button>
		</div>
	{/if}
